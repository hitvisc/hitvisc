...
# Usage:
USAGE_TEMPLATE="hitvisc_update_search.sh FRONT_ENTITY_ID NAME USAGE_TYPE DESCRIPTION"
...
source "$DIR/update_search.sh" "$@"
EXITCODE=$?
if [ $EXITCODE -ne $CODESUCCESS ]; then
  let ERRCODE=$EXITCODE-256; echo $ERRCODE; exit $EXITCODE; fi

if [ -z $SEARCH_ID ] || [ $SEARCH_ID -le 0 ]; then echo $CODEOTHERERR; exit $CODEOTHERERR; fi
