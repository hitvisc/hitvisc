...
# Arguments: FRONT_TARGET_ID NAME USAGE_TYPE DESCRIPTION
...
if [[ $TARGET_ID -gt 0 ]]; then
  PSQL_STATUS=$(echo "UPDATE registry.target SET name = '$TARGET_NAME', usage_type = '$TARGET_USAGE_TYPE', description = '$TARGET_DESC' WHERE id = $TARGET_ID" | psql --dbname=hitvisc -qtA)
else
  log_msg_error "unable to update target (cmd: UPDATE registry.target SET name = '$TARGET_NAME', usage_type = '$TARGET_USAGE_TYPE', description = '$TARGET_DESC' WHERE id = $TARGET_ID)"; return $CODEPSQLERR; fi
