...
useradd -m --shell /bin/bash ansible && usermod -a -G sudo ansible \
 && echo "ansible ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers && apt-get update && apt install -y git vim 
...
6. Завершить подготовку Системы на _рабочем компьютере_.

```
# Убедиться, что находимся в рабочей директории: например, /home/user/work
cd hitvisc/install
ansible-playbook -K hitvisc-finalize.yml
```
...
```
(по умолчанию 8080). В веб-интерфейсе доступна как регистрация новой учетной записи, так и вход по предустановленной учетной записи (adm.hitvisc@yandex.ru, password).

> [!IMPORTANT]
> Для работы системы необходимо разрешить входящий TCP-трафик на порты 80, 8080 и 9090.
...
- Закрытый – проект не публикуется в системе для остальных пользователей; 
...
- Частные ресурсы – ресурсы, добавленные пользователем из собственных доступных;
- Общие ресурсы – ресурсы других пользователей системы, отмеченные как общедоступные, ресурсы волонтеров.

Заключительный шаг создания проекта – это его запуск. На этом шаге пользователю предоставляется сводка всех параметров проекта и предлагается отправить проект на запуск расчетов. 
...
Созданный ранее сохраненный проект можно отредактировать и/или удалить. Для этого в интерфейсе системы, в основном окне списка проектов, на карточке соответствующего проекта необходимо выбрать пункт «Редактировать проект» или «Удалить проект». Редактирование доступно только для проектов, принадлежащих данному пользователю и имеющих статус "доступен". Редактировать можно название, режим доступа и описание. Функционал редактирования проекта аналогичен функционалу создания проекта на Шаге 1 с заполненными ранее полями. Удаление доступно только для проектов, принадлежащих данному пользователю. При удалении проект переводится в статус "архивирован".
...
Для вычислительных узлов, добавленных пользователем, доступна возможность редактирования имени узла и типа.
...
- общее количество завершенных узлом заданий, 
- режим доступа к вычислительному узлу.
Для вычислительных узлов, добавленных пользователем, доступна возможность редактирования имени узла. Тип тестового ресурса изменить нельзя.

### Добавление ресурсов

Доступно добавление вычислительных узлов. Новый вычислительный узел добавляется с типом "частный". При необходимости, тип узла может быть изменен на странице «Ресурсы-Частные».
...
Для добавления новой библиотеки мишени необходимо нажать кнопку «Добавить» главного рабочего окна и заполнить соответствующую форму.
...
### Редактирование и удаление мишеней

Созданную ранее сохраненную мишень можно отредактировать и/или удалить. Для этого в интерфейсе системы, в основном окне списка мишеней, на карточке соответствующей мишени необходимо выбрать пункт «Редактировать мишень» или «Удалить мишень». Редактирование доступно только для мишеней, принадлежащих данному пользователю и имеющих статус "доступна". Редактировать можно название, режим доступа, описание, автора и источник. Функционал редактирования мишени аналогичен функционалу создания мишени с заполненными ранее полями. Удаление доступно только для мишеней, принадлежащих данному пользователю. При удалении мишень переводится в статус "архивирована".
...
Для библиотек, созданных пользователем, доступны редактирование и удаление библиотеки.
Для добавления новой библиотеки лигандов необходимо нажать кнопку «Добавить» главного рабочего окна и заполнить соответствующую форму, загрузив в систему библиотеку в формате SDF.
При заполнении формы необходимо указать следующую информацию:
...
### Редактирование и удаление библиотек лигандов

Созданную ранее сохраненную библиотеку лигандов можно отредактировать и/или удалить. Для этого в интерфейсе системы, в основном окне списка мишеней, на карточке соответствующей библиотеки лигандов необходимо выбрать пункт «Редактировать коллекцию лигандов» или «Удалить коллекцию лигандов». Редактирование доступно только для библиотек лигандов, принадлежащих данному пользователю и имеющих статус "доступна". Редактировать можно название, режим доступа, описание, автора и источник. Функционал редактирования библиотеки лигандов аналогичен функционалу создания библиотеки лигандов с заполненными ранее полями. Удаление доступно только для библиотек лигандов, принадлежащих данному пользователю. При удалении библиотека лигандов переводится в статус "архивирована".
